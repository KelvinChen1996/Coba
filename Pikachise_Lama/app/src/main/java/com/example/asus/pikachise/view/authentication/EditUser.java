...
public class EditUser extends AppCompatActivity {
    ...
    private void sendEdittoApi(){
        ...
        if(mediaPath!=null){
            ...
            service.changeuserimageRequest(requestToken, body)
                    .enqueue(new Callback<ResponseBody>() {
                        @Override
                        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                            if (response.code()==401 || response.code()==400){
                                ...
                                try{
                                    JSONObject jsonResults = new JSONObject(response.body().string());
                                    if(jsonResults.getString("message").equals("Image changed successfully")){
        ...
        service = apiUtils.getAPIService();
        service.editProfile(requestToken, requestName, requestAddress, requestPhoneNumber).
                enqueue(new Callback<ResponseBody>() {
                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        if (response.code()==401 || response.code()==400){
